...
G = robustcontrol.mimotf(C*robustcontrol.mimotf(s*np.eye(2) - A).inverse()*B + D)

# a) Controllability analysis

G_poles = G.poles()
G_rhp_poles = robustcontrol.RHPonly(G_poles)
# G has stable poles, -10 with multiplicity of 2 and -1
G_zeros = G.zeros()
G_rhp_zeros = robustcontrol.RHPonly(G_zeros)
...
robustcontrolplot.rga_nm_plot(G, pairing_list=[pairing1, pairing2], 
