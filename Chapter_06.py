...
def Equation_6_8_input(error_poles_direction, usedeadtime=False):
    ...
    if usedeadtime:
        ...
        # each outputs' maximum dead time.
        # This would ensure tighter bounds on T and S.
        # The minimum function is used because all stable systems has dead
        # time with a negative sign.
        ...
        def Dead_time_matrix(s, dead_time_vec_max_row):
            ...
            dead_time_matrix = (np.diag(np.exp(
                                np.multiply(dead_time_vec_max_row, s))))
            return dead_time_matrix
        ...
        for j in range(len(Poles_G)):
            for j in range(len(Poles_G)):
                denominator_mat = (np.transpose(np.conjugate(
                                   up_direction[:, i])) *
                   Dead_time_matrix(Poles_G[i], dead_time_vec_max_row) *
                   Dead_time_matrix(Poles_G[j], dead_time_vec_max_row) *
                   up_direction[:, j])
