...
# The current values are for a specific system (system reference unknown).
...
def reference_change():
    """
    Reference change matrix/vector for use in eq 6-52 pg 242
...
def Min_Peaks(Zeros_G, Poles_G, error_poles_direction, io_type, usedeadtime=False):
    """ 
    Equation 6.8    
    This function will calculate the minimum peak values of S and T if the 
    system has zeros and poles in the input or output. Systems with deadtime
    ...
    Poles_G : numpy array
        Direction vector.
    error_poles_direction : numpy array
        Direction verctor error.
    io_type : string
        Specific directional vector type ['input','output']
    usedeadtime : boolean
        Is there deadtime in the system (optional).
    ...
    z_direction = np.matrix(np.zeros([G(0.001).shape[0], len(Zeros_G)]))
    ...
    Qz = (z_direction.H * z_direction)/(yz_mat1 + yz_mat2)

    yp_mat1 = (np.matrix(np.diag(Poles_G)) *
               np.matrix(np.ones([len(Poles_G), len(Poles_G)])))
    yp_mat2 = yp_mat1.T

    Qp = (p_direction.H * p_direction) / (yp_mat1 + yp_mat2)
    ...
    Qzp = z_direction.H * p_direction / (yzp_mat1 - yzp_mat2)
    ...
    if usedeadtime:
        ...
        # Final calculation of the peak value
    ...
    if usedeadtime:
        ...
        # The minimum function is used because all stable systems has dead time
        ...
        def Dead_time_matrix(s, dead_time_vec_max_row):
            ...
            dead_time_matrix = np.diag(np.exp(np.multiply(
        ...
        print 'Dead time per output channel is for the worst case dead\
               time in that channel'
        print 'Ms_min = Mt_min = ', Ms_min
        print ''
...
def Equation_6_24(error_poles_direction):
    # This function is for equation 6-24 in Skogestad.
    ...
    KS_PEAK = [np.linalg.norm(
               np.linalg.svd(G_s(RHP_p+error_poles_direction))[2][:, 0].H *
               np.linalg.pinv(G_s(RHP_p+error_poles_direction)), 2)
...
def Equation_6_43(w_start, w_end):
    ...
    # This function calculated the disturbance condition number
    # This number is ideally close to one
    # If this number is close to the condition number of the plant means
    # that the disturbances has a greater ability to effect the output.

    w = np.logspace(w_start, w_end, 1000)

    dist_condition_num = [np.linalg.svd(G(1j * w_i))[1][0] *
                          np.linalg.svd(np.linalg.pinv(G(1j * w_i)) *
                          np.linalg.svd(Gd(1j * w_i))[1][0] *
                          np.linalg.svd(Gd(1j * w_i))[0][:, 0])[1][0]
                          for w_i in w]
    condition_num = [np.linalg.svd(G(1j * w_i))[1][0] *
                     np.linalg.svd(np.linalg.pinv(G(1j * w_i)))[1][0]
                     for w_i in w]
...
def Equation_6_48(error_zeros_direction):
    ...
    RHP_alignment = (np.array([
                     np.abs(np.linalg.svd(G(
                     RHP_Z + error_zeros_direction))[0][:, -1].H *
                         np.linalg.svd(
                             Gd(RHP_Z+error_zeros_direction)[:, c])[1][0] *
                         np.linalg.svd(
                             Gd(RHP_Z+error_zeros_direction)[:, c])[0][:, 0])
                     for RHP_Z in Zeros_G for c in range(columns)]))
    print RHP_alignment
    print ''
...
def Equation_6_50(w_start, w_end, matrix='single'):
    # If matrix is equal to single then just a single disturbance
    # in Gd is considered.
    ...
    if matrix == 'single':
        ...
        for column in range(columns):
            mod_invG_gd = [np.max(np.linalg.pinv(G(1j * w_i)) *
                           Gd(1j * w_i)[:, column]) for w_i in w]
            plt.figure(count)
    ...
    if matrix == 'multiple':
        mod_invG_Gd = [np.max(np.linalg.pinv(G(1j * w_i)) * Gd(1j * w_i))
                       for w_i in w]
        plt.semilogx(w, mod_invG_Gd, 'b')
        plt.semilogx([w[0], w[-1]], [1, 1], 'r')
    plt.show()
...
def Equation_6_52(w_start, w_end, R, wr, type_eq='minimal'):
    # This function is from equation 6-52 and 6-53 from Skogestad
    # The one is the minimal requirement for input saturation check in terms
    # of set point tracking .
    # The other is the more tighter bounds and check type is to specify
    # if the minimal requiremant wants to be checked
    ...
    if type_eq == 'tighter':
        min_singular_invR_G = (np.array([np.linalg.svd(
                               np.linalg.pinv(R) * G(1j * w_i))[1][-1]
                               for w_i in w]))
        plt.loglog(w, min_singular_invR_G, 'b')
        plt.loglog([w[0], w[-1]], [1, 1], 'r')
        plt.loglog([wr, wr], [0.8 * np.min(min_singular_invR_G), 1.2 *
                   np.max(min_singular_invR_G)], 'g')
        plt.loglog(w[0], 1.1)
        plt.loglog(w[0], 0.8)
...
def Equation_6_55(w_start, w_end):
    ...
    print 'For the graphs thats generated the blue line\
           needs to be above the red line'
    print 'If the red line is above, input saturation is going to occur for\
           acceptable control with load rejection'
