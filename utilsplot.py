...
your_utilsplot_functionA(G, w_start=-5, w_end=2, 
                         axlim=[None, None, 0, 1], more_parameters)
plt.show()
...
def rga_plot(G, w_start=-2, w_end=2, axlim=None, points=1000, fig=0,
    ...
    >>> def G(s):
    ...     G = 0.01**(-5*s)/((s + 1.72e-4)*(4.32*s + 1))*numpy.matrix(
...
def rga_nm_plot(G, pairing_list=None, pairing_names=None, w_start=-2,
    ...
    >>> rga_nm_plot(G, [pairing], w_start=-5,
    w_end=2, axlim=[None, None, 0., 1.])
