...
def PEAK_MIMO(w_start,w_end,error_poles_direction,wr):
    ...
    store_input_accept_max      = np.zeros(len(w))
    store_input_accept_min      = np.zeros(len(w))
    store_singular_accept_min    = np.zeros(len(w))
    store_singular_accept_max   = np.zeros(len(w))
    
    for i in range(len(w)):
        #this considers the worst direction and magnitude that gd could achieve on both the maximum and minimum singular values of G
        store_input_accept_max[i]       = np.abs(np.transpose(np.conjugate(np.linalg.svd(G(w[i]))[2][:,0]))*np.max(np.linalg.svd(Gd(w[i]))[1])*np.linalg.svd(Gd(w[i]))[0][:,0])-1
        store_input_accept_min[i]       = np.abs(np.transpose(np.conjugate(np.linalg.svd(G(w[i]))[2][:,-1]))*np.max(np.linalg.svd(Gd(w[i]))[1])*np.linalg.svd(Gd(w[i]))[0][:,0])-1
        store_singular_accept_min[i]    = np.max(np.linalg.svd(G(w[i]))[1])
        store_singular_accept_max[i]    = np.min(np.linalg.svd(G(w[i]))[1])
     
    
    print 'Figure 2 is the singular value of an specific output with input and disturbance direction vector'
    ...
    print ''
    plt.figure(2)
    plt.title('Checking input saturation for Acceptable control MAXIMUM singular')
    plt.subplot(211)
    plt.semilogx(w,store_input_accept_max,'r')
    plt.semilogx(w,store_singular_accept_max,'b')
    plt.subplot(212)
    plt.title('Checking input saturation for Acceptable control MINIMUM singular')
    plt.semilogx(w,store_input_accept_min,'r')
    plt.semilogx(w,store_singular_accept_min,'b')
    ...
    print 'Figure 3 is to check input saturation for reference changes'
    ...
    print 'Figure 4 is the maximum and minimum singular values of G over a frequency range'    
    print ''
    plt.figure(4)
    plt.title('min_singular(G(jw)) and max_singular(G(jw))')
