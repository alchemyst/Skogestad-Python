...
def PEAK_MIMO(w_start,w_end,error_poles_direction):
    ...
    if np.sum(Zeros_G)!=0:
        if np.sum(Poles_G)!=0:   
            ...
            yz_direction = np.zeros([len(Zeros_G),len(Zeros_G)])
            yp_direction = np.zeros([len(Poles_G),len(Poles_G)])
            
            
            for i in range(len(Zeros_G)):
                [U,S,V]              = np.linalg.svd(G(Zeros_G[i]))
                yz_direction[:,i]    = U[:,-1]
    ...
    [U_gd,S_gd,V_gd] = np.linalg.svd(Gd(0.0000))
    y_gd_max         = U_gd[:,0]
    mod_G_gd_ss      = np.max(np.linalg.inv(G(0.0000))*y_gd_max)
    ...
    check_inputsaturation_dist  = np.max(np.linalg.inv(np.abs(G(w*1j))))
    
    mod_G_gd    = np.zeros(len(w))
    
    for i in range(len(w)):
        [U_gd,S_gd,V_gd]    = np.linalg.svd(Gd(w[i]))
        gd_m                = U_gd[:,0]
        mod_G_gd[i]         = np.max(np.linalg.inv(G(w[i]))*gd_m)
    
    def G_gd(w):
        [U_gd,S_gd,V_gd]    = np.linalg.svd(Gd(w))
        gd_m                = U_gd[:,0]
        mod_G_gd[i]         = np.max(np.linalg.inv(G(w))*gd_m)-1
        return mod_G_gd
