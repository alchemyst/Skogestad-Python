...
class tf(object):
    ...
    Traceback (most recent call last):
        ...
    ValueError: Transfer functions can only be added if their deadtimes 
    are the same. self=tf([ 1.], [ 2.  1.]), other=tf([ 1.], [ 1.  1.], 
    deadtime=2)
    ...
    def __add__(self, other):
        ...
        not (self.zerogain or other.zerogain):
            raise ValueError("Transfer functions can only be added if their \
            deadtimes are the same. self={}, other={}".format(self, other))
...
class mimotf(object):
    ...
    def cofactor_mat(self):
        ...
        for i in range(m):
            for j in range(n):
                minorij = det(numpy.delete(numpy.delete(A, i, axis=0), \
                j, axis=1))
            ...
            for t in timedata:
                ...
                if constraint is not None:
                    if (y1[0, 0] > constraint) or bconst:
                        ...
                        bconst = True  
                        # TODO : incorrect, find correct switching condition
                        u = 0  
                    dxdt2 = A2*x2 + B2*u
                    y2 = C2*x2 + D2*u
                    x2 = x2 + dxdt2 * dt
...
def marginsclosedloop(L):
    ...
    # calculate the freqeuncy at |S(jw)| = 0.707 from below (Search from 0)
    wb = optimize.fsolve(modS, 0)
    # calculate the freqeuncy at |T(jw)| = 0.707 from above (Search from 1)
...
def sym2mimotf(Gmat):
    """Converts a MIMO transfer function system in sympy.Matrix 
    form to a mimotf object making use of individual tf objects.
    ...
    # Make Empty list of lists. This will be appended to form mimotf input list
...
def feedback_mimo(forward, backward=None, positive=False):
    ...
    if backward.shape[1] != forward.shape[0]:
        raise ValueError("The column dimension of backward matrix must equal \
        row dimension of forward matrix")
...
def tf2ss(H):
    ...
    representation. This method and the examples were obtained from course 
    work notes available at 
    ...
    for j in range(Hcols):
        ...
        for i in range(Hrows):
            ...
            if H[i, j].numerator.order == H[i, j].denominator.order:  
                # approximate the limit as s->oo for transfer function elements
                Hinf[i, j] = H[i, j].numerator.coeffs[0]/ \
                H[i, j].denominator.coeffs[0]
            elif H[i, j].numerator.order > H[i, j].denominator.order:
                return 'Enter a matrix of stricly proper transfer functions'

        d[j] = tf(lcm)  # convert lcm to a tf object
        mu[j] = lcm.order
        D[j, j] = d[j]  # create a diagonal matrix of lcms
        # create a list of coeffs of the lcm for that column
        Lvect[j] = list((d[j].numerator.coeffs[1:]))  
        Lvect[j].reverse()
        # create a block diagonal matrix from the list of lists
        L = sc_linalg.block_diag(L,Lvect[j])  
    ...
    # This loop generate the M matrix, which forms the output matrix, C
    ...
    for order in mu:
        # Constructs an off diagonal matrix used to form the state matrix
        Acbar = sc_linalg.block_diag(Acbar, offdiag(order)) 
        # Constructs a lower diagonal matrix which forms the input matrix
        Bcbar = sc_linalg.block_diag(Bcbar, lowerdiag(order))  
...
def kalman_controllable(A, B, C):
    """Computes the Kalman Controllable Canonical Form of the input 
       system A, B, C, making use of QR Decomposition.
       Can be used in sequentially with kalman_observable to obtain a 
       minimal realisation.
    Parameters
    ----------
    A : numpy matrix
    ...
    factor : int
        The number of additional significant digits after the first 
        significant digit to round the returned matrix elements to.
...
def kalman_observable(A, B, C):
    """Computes the Kalman Observable Canonical Form of the inout system
       A, B, C, making use of QR Decomposition.
       Can be used in sequentially with kalman_controllable to obtain 
...
# Remove_states = remove_uncontrollable_or_unobservable_states
def remove_states(a, b, c, con_or_obs_matrix, uncontrollable=True, 
                  unobservable=False, rank=None):
    ...
    :param rank: optional (int)
        ...
        if the rank is available set the rank=(rank of matrix) to avoid 
        calculating matrix rank twice
    ...
    and the unobservable states are removed the
    matrices Aco, Bco and Cco 
    (the controllable and observable subspace of A, B and C) will be returned

    If the observable subspace of A, B and C are given (Ao, Bo and Co) 
    and the uncontrollable states are removed the
    matrices Aco, Bco and Cco 
    (the controllable and observable subspace of A, B and C) will be returned
    ...
    >>> Ac, Bc, Cc = remove_states
    (A, B, C, controllability_matrix)
    ...
    >>> Ao, Bo, Co = remove_states
    (Ac, Bc, Cc, observability_matrix, uncontrollable=False, unobservable=True)
    ...
    ...
    # calculate the difference between the number of states and the number 
    # of controllable or observable states
    m = n_states - rank

    # if system is already state controllable 
    # or observable return matrices unchanged
    if m == 0:
        return a, b, c

    # create the a matrix P with dimensions n_states x n_states used to 
    # change matrices A, B and C to the Kalman Canonical Form
    ...
    if uncontrollable is True and unobservable is False:
        ...
        # this matrix will replace all the dependent columns 
        # in P to make P invertible
        replace_matrix = numpy.matrix(numpy.random.random((n_states, m)))

        # make P invertible
        P[:, rank:n_states] = replace_matrix

        # When removing the uncontrollable states the constructed matrix P is 
        # actually the inverse of P (P^-1) and
...
def minimal_realisation(a, b, c):
    """"This function will obtain a minimal realisation for a state 
    space model in the form given in Skogestad
    ...
    if rank_C <= rank_O:
        Ac, Bc, Cc = remove_states(a, b, c, C, rank=rank_C)

        O = state_observability_matrix(Ac, Cc)

        Aco, Bco, Cco = remove_states(Ac, Bc, Cc, O, uncontrollable=False,
                                      unobservable=True)

    else:
        Ao, Bo, Co = remove_states(a, b, c, O, uncontrollable=False,
                                   unobservable=True, rank=rank_O)

        _, _, C = state_controllability(Ao, Bo)

        Aco, Bco, Cco = remove_states(Ao, Bo, Co, C)
...
def minors(G, order):
    ...
    for rowstokeep in itertools.combinations(range(Nrows), order):
        for colstokeep in itertools.combinations(range(Ncols), order):
            ...
            elif (type(G_slice) == mimotf) and \
            (G_slice.shape[0] == G_slice.shape[1]):
...
def zeros(G=None, A=None, B=None, C=None, D=None):
    ...
    '''
    # TODO create a beter function to accept parameters 
    # and switch between tf and ss
...
def BoundST(G, poles, zeros, deadtime=None):
    ...
    if deadtime is None:
        ...
        pre_mat = sc_linalg.sqrtm((numpy.linalg.inv(Qz))) \
        .dot(Qzp).dot(sc_linalg.sqrtm(numpy.linalg.inv(Qp)))
        # Final equation 6.8
        ...
        def Dead_time_matrix(s, dead_time_vec_max_row):
            dead_time_matrix = numpy.diag(numpy 
            .exp(numpy.multiply(dead_time_vec_max_row, s)))
